 ...
 * @author Rob Graff
 */
public class HeaderRequestAccessor implements Accessor<Header, ServiceCall>, ServiceAssertionActualProvider<String> {
	 ...
	 * @param key the key
	 */
	public HeaderRequestAccessor(ServiceCallReference callRef, String key){
	 ...
	 * @param key the key
	 */
	public HeaderRequestAccessor(String key){
		this.key = key;
	}
	...
	public String getActual(ServiceCall call) {
		return call.getHttpRequest().getHeaders().get(key).getValues().get(0);
	}
	...
	@Override
	public Header read() {
		return this.call.getHttpRequest().getHeaders().get(key);
	}
